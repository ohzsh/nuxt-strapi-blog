...
const { data: res } = await useAsyncData(() => find<{
  /**
   * 话题
   */
  title: string
...
const { curTopicId, toggle } = useTopics()
</script>

<template>
  <header class="p-4 sticky z-10 top-0 bg-white dark:bg-gray-800 dark:text-gray-100">
    <div class="container flex justify-between h-10 mx-auto">
      <a rel="noopener noreferrer" href="#" aria-label="Back to homepage" class="flex items-center p-2">
        <LayoutToggle>
          <svg
            xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 32 32" class="w-8 h-8 dark:text-violet-400 transition-all-500 transform hover:rotate-365"
          >
      ...
      <ul class="items-stretch hidden space-x-3 lg:flex">
        <li
          v-for="item in res?.data" :key="item.id" class="flex"
          @click="toggle(item.id)"
        >
          <a
            rel="noopener noreferrer" href="#" class="flex items-center px-4 -mb-1 border-b-2 dark:border-transparent"
            :class="{ 'border-black dark:text-violet-400 dark:border-violet-400': curTopicId === item.id }"
          >
